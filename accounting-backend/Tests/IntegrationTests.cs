...
public class IntegrationTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task Organisation_Create_And_Access()
    {
        ...
        // list endpoint should return org
        var listResp = await client.GetAsync("/api/organisations");
        listResp.EnsureSuccessStatusCode();
        var listContent = await listResp.Content.ReadFromJsonAsync<JsonElement>();
        listContent.ValueKind.Should().Be(JsonValueKind.Array);
        listContent[0].GetProperty("id").GetGuid().Should().Be(orgId);
    }
}
